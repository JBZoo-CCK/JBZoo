...
class JBUpload
{
    ...
    protected function gd_watermark($file_name)
    {
        ...
        // Watermark position
        if ($this->options['watermark_position'] == 'tile')
        {
            $new_watermark_height = imagesy($src_img) / 1.5;
            $new_watermark_width  = $new_watermark_height / $watermark_ratio;
            ...
            imagesettile($src_img, $new_watermark);
            imagefilledrectangle($src_img, 0, 0, imagesx($src_img), imagesy($src_img), IMG_COLOR_TILED);
        }
        else
        {
            $margin_right         = 20;
            $margin_top           = 20;
            $new_watermark_width  = imagesx($src_img) * 0.2;
        ...
        $result = $write_func($src_img, $file_path, $image_quality);
    ...
    {
        if ($this->request->files->get($id))
        {
            return $this->request->files->get($id)[0];
        }
    ...
    {
        return $this->request->post->get($id);
    }
    ...
    public function generate_response($content, $print_response = true)
    {
        ...
        if ($print_response)
        {
            ...
            // $this->body($json); // todofixj4 WTF
        }

        jexit($json); //todoj4fix
        return $json;  //todoj4fix no need i know :)
        // return $content;
    }
    ...
    public function delete($print_response = true)
    {
        ...
        {
            $file_path = $this->get_upload_path($file_name);
            // $success   = JSession::checkToken('get') && is_file($file_path) && $file_name[0] !== '.' && unlink($file_path);  //todofixj4
            $success   = Joomla\CMS\Session\Session::checkToken('get') && is_file($file_path) && $file_name[0] !== '.' && unlink($file_path);
            if ($success)
            {
                foreach ($this->options['image_versions'] as $version => $options)
                {
